...
func _build_stats_panel() -> void:
	...
	stats_panel.offset_left = -120.0
	stats_panel.offset_top = 10.0
	stats_panel.offset_right = 120.0
	...
	vbox.custom_minimum_size = Vector2(472, 0)
	...
	_health_bar = _make_bar(vbox, "Health", Color(0.2, 0.8, 0.3), font)
	_spoil_bar  = _make_bar(vbox, "Fresh",  Color(0.6, 0.9, 0.2), font)
...
func _update_stats_panel() -> void:
	_info_label.text = str("nutrition: ", snappedf(size, 0.01))
	_health_bar.max_value = 5.0 * size
	_health_bar.value = health
	if main.plant_spoil_time > 0:
		_spoil_bar.max_value = main.plant_spoil_time
